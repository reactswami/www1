...
export default ({ id, disclosure }: Props) => {
   ...
   const getTitle = () => {
      if (id.length === 0) {
         return 'Delete all configured schedules';
      }

      if (id.length === 1) {
         return `Delete Schedule - ${scheduleName}`;
      }
   ...
   const getConfirmation = () => `Are you sure you want to delete ${count} schedule${suffix}? This action cannot be undone`;
